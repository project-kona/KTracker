...
#if 0
  htable->tail = 0;
  htable->table = malloc(TABLE_SIZE * sizeof (uint64_t));
  htable->bitmap = malloc(TABLE_SIZE * sizeof(uint64_t));
  htable->table[0] = 0;
  htable->bitmap[0] = 0;
#else 
...
#if 0
  int tail = htable->tail;
  int N = (vaddr - page_vaddr) / CL_SIZE;
  if ((tail > 0) && (htable->table[tail - 1] == page_vaddr)) {
    /* same page */
    htable->bitmap[tail-1] |= (1ULL << N);
    ...
    htable->table[tail] = page_vaddr;
    htable->bitmap[tail] = (1ULL << N);
    htable->tail++;
  }
#else 
...
void htable_print(htable_t *htable, int fd, uint64_t iter) {
  ...
  if (htable->tail > 0) {
    write(fd, &iter, 8);
    write(fd, &(htable->tail), 8);

    //printf("iter %lu tail %lu\n", iter, htable->tail);

    for (int i = 0; i < htable->tail; i++) {
      write(fd, &(htable->table[i]), 8);
      write(fd, &(htable->bitmap[i]), 8);
      //printf("%lx %lu\n", htable->table[i], htable->bitmap[i]);
    }
  }
#else
  uint64_t count = htable->tble[HTABLE_IDX_COUNT];
  size_t size = 2*sizeof(uint64_t)*(count+1);
  //printf("htable_print writing %lu size %lu\n", count, size); 
  if (count > 0) {
